...
{
  void write_sa(char *filename, unsigned int *p, unsigned int n);
  ...
  fprintf(stderr,"alloc %d bytes for sa, sizeof(*p)=%d\n",(n)*sizeof *p,sizeof *p);
...
/* **********************************************************
   ...
   ********************************************************** */
void write_sa2(char *filename, int *p, int n)
{
  int int_log2(int);
...
void write_sa3(char *filename, unsigned int *p, unsigned int n)
{
...
//  unsigned int a;
  ...
  for (i=0;i<n;i++) p[i]=htonl(p[i]); 

  fwrite(p,sizeof(*p),n,sa);
...
void write_sa(char *filename, unsigned int *p, unsigned int n)
{
  #define SA_WBUFSIZE 65536
  int sa_fd;
  unsigned int i,step,pos;
//  unsigned int a;

  if(_ds_Verbose)
    fprintf(stderr,"Writing sa to file %s\n",filename);

  if((sa_fd=creat(filename,S_IRGRP|S_IRWXO|S_IWUSR|S_IRUSR))<0)
    perror(filename);
  
  for (i=0;i<n;i++) p[i]=htonl(p[i]); 

  step=SA_WBUFSIZE/sizeof(*p);
  pos=0;
  while (pos<n) {
    write(sa_fd,(p+pos),sizeof(*p)*(step<(n-pos)?step:n-pos));
    pos+=step;
  } 
